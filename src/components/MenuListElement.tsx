...
import { add_toPlayList, selectPlayList, sellectMusic, setsellect } from "../redux/Slice";
    ...
    >
      <div className="flex w-[calc(100%-25px)]">
        ...
        <div className="flex flex-col w-[(calc(100%-32px)]">
          <p
            className={
              (add ? "w-[200px]" : "max-w-[95px] group-hover:w-[70px]") +
              " text-stone-50  overflow-hidden   whitespace-nowrap text-ellipsis "
            }
          >
            {data.name}
          </p>
      ...
      {add ? (
        <IoIosAddCircleOutline className="text-[24px]   hidden group-hover:flex" />
      ) : (
        <Link
          to={"/playList"}
          onClick={() => {
